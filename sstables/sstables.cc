...
static entry_descriptor make_entry_descriptor(sstring sstdir, sstring fname, sstring* const provided_ks, sstring* const provided_cf) {
    ...
    return entry_descriptor(sstdir, ks, cf, generation::type{boost::lexical_cast<unsigned long>(generation)}, version, sstable::format_from_sstring(format), sstable::component_from_sstring(version, component));
}
