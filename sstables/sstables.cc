...
sstring sstable::component_basename(const sstring& ks, const sstring& cf, version_types version, generation::type generation,
...
sstring sstable::filename(const sstring& dir, const sstring& ks, const sstring& cf, version_types version, generation::type generation,
                          format_types format, component_type component) {
...
future<> sstable::check_create_links_replay(const sstring& dst_dir, generation::type dst_gen,
...
future<> sstable::create_links_common(const sstring& dir, generation::type generation, bool mark_for_removal) const {
...
future<> sstable::create_links(const sstring& dir, generation::type generation) const {
    return create_links_common(dir, generation, false /* mark_for_removal */);
}

future<> sstable::create_links_and_mark_for_removal(const sstring& dir, generation::type generation) const {
    return create_links_common(dir, generation, true /* mark_for_removal */);
}

future<> sstable::set_generation(generation::type new_generation) {
...
future<> sstable::move_to_new_dir(sstring new_dir, generation::type new_generation, bool do_sync_dirs) {
    ...
    _dir = new_dir;
    generation::type old_generation = std::exchange(_generation, new_generation);
...
static entry_descriptor make_entry_descriptor(sstring sstdir, sstring fname, sstring* const provided_ks, sstring* const provided_cf) {
    ...
    return entry_descriptor(sstdir, ks, cf, generation::from_value(boost::lexical_cast<unsigned long>(generation)), version, sstable::format_from_sstring(format), sstable::component_from_sstring(version, component));
}
...
delete_atomically(std::vector<shared_sstable> ssts) {
    ...
    return seastar::async([ssts = std::move(ssts)] {
        sstring sstdir;
        min_max_tracker<generation::type> gen_tracker;
