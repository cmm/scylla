...
sstable_directory::handle_component(scan_state& state, sstables::entry_descriptor desc, fs::path filename) {
    if ((generation::value(desc.generation) % smp::count) != this_shard_id()) {
        return;
    }
...
sstable_directory::process_sstable_dir(const ::io_priority_class& iop, bool sort_sstables_according_to_owner) {
    ...
    _max_generation_seen =  boost::accumulate(state.generations_found | boost::adaptors::map_keys, generation::type(0), [] (generation::type a, generation::type b) {
        return std::max<generation::type>(a, b);
    });
    ...
    co_await parallel_for_each_restricted(state.descriptors, [this, sort_sstables_according_to_owner, &state, &iop] (std::tuple<generation::type, sstables::entry_descriptor>&& t) {
