...
public:
    ...
    const reader_permit& permit() const noexcept { return _permit; }
    ...
    flat_mutation_reader_v2 get_underlying() && noexcept {
...
future<> consume_partitions(mutation_fragment_v1_stream& stream, Consumer consumer) {
    while (true) {
        auto mo = co_await read_mutation_from_flat_mutation_reader(stream);
        if (!mo) {
            co_return;
        }
        if (consumer(std::move(*mo)) == stop_iteration::yes) {
            co_return;
        }
    }
};
