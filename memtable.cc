...
memtable::make_flat_reader(schema_ptr s,
    ...
    if (query::is_single_partition(range) && !fwd_mr) {
        ...
        auto rd = make_partition_snapshot_flat_reader<partition_snapshot_read_accounter>(snp_schema, std::move(permit), std::move(dk),
                                                                                         std::move(cr), std::move(snp), digest_requested,
