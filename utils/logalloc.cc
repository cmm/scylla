...
class reclaim_timer {
    ...
    std::function<void ()> _outer_stall_report_function;
...
public:
    reclaim_timer(is_preemptible preemptible, size_t memory_to_release, size_t reserve_segments, tracker::impl& tracker)
        ...
        , _outer_stall_report_function(engine().get_stall_detector_report_function())
    {
        if (_debug_enabled) {
            _start = clock::now();
        ...
        engine().set_stall_detector_report_function([this] { on_stall(); });
    }
...
private:
    void on_stall() {
        ...
        if (!_debug_enabled) {
            // we did not measure time at the beginning, so let's
            // boldly assume that exactly the detection threshold
            // elapsed since then
            _start = clock::now() - engine().get_blocked_reactor_notify_ms();
        }
    }
    ...
    void report() const noexcept {
        ...
        if (_memory_released > 0) {
            auto bytes_per_second =
                static_cast<float>(_memory_released) / std::chrono::duration_cast<std::chrono::duration<float>>(duration).count();
...
tracker::impl::impl() {
    ...
    _metrics.add_group("lsa", {
        ...
        sm::make_derive("memory_freed", [] {
            return shard_segment_pool.statistics().memory_freed - shard_segment_pool.statistics().memory_evicted;
        }, sm::description("Counts number of bytes which were requested to be freed in LSA, other than during eviction.")),
    });
}
