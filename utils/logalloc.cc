...
public:
    reclaim_timer(is_preemptible preemptible, size_t memory_to_release, size_t reserve_segments, tracker::impl &tracker):
        _preemptible(preemptible), _memory_to_release(memory_to_release), _reserve_segments(reserve_segments), _tracker(tracker)
    {
        if (_debug_enabled) {
            _start = clock::now();
    ...
    ~reclaim_timer() {
        engine().set_stall_detector_report_function(_outer_stall_report_function);
        if (_debug_enabled || _stall_reported) {
            report();
        }
    }
...
private:
    ...
    bool _debug_enabled = timing_logger.is_enabled(logging::log_level::debug);
    bool _stall_reported = false;
    std::function<void ()> _outer_stall_report_function = engine().get_stall_detector_report_function();
    ...
    std::pair<const char *, duration_type> get_duration() const noexcept {
        ...
        if (_debug_enabled) {
            ...
            return {"~", now - _start + engine().get_blocked_reactor_notify_ms()};
        }
    }
    ...
    void report() const noexcept {
        auto time_level = _stall_reported ? log_level::warn : log_level::debug;
        auto info_level = _stall_reported ? log_level::info : log_level::debug;
        ...
        timing_logger.log(time_level, "Reclamation cycle took {}{} us, trying to release {:.3f} MiB {}preemptibly",
                          ...
                          (float)_memory_to_release / MiB, _preemptible ? "" : "non-");
        log_if_any(info_level, "reserved segments", _reserve_segments);
        if (_memory_released > 0) {
            auto bytes_per_second =
                static_cast<float>(_memory_released) / std::chrono::duration_cast<std::chrono::duration<float>>(duration.second).count();
...
tracker::impl::impl() {
    ...
    _metrics.add_group("lsa", {
        ...
        sm::make_derive("memory_evicted", [this] { return shard_segment_pool.statistics().memory_evicted; },
                        sm::description("Counts number of bytes which were evicted as part of memory reclamation.")),
        ...
        sm::make_derive("memory_freed_manually", [this] {
            return shard_segment_pool.statistics().memory_freed - shard_segment_pool.statistics().memory_evicted;
        }, sm::description("Counts number of bytes which were requested to be freed in LSA other than by eviction.")),
    });
}
