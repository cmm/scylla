...
future<std::pair<bool, bool>> large_data_handler::maybe_record_large_partitions(const sstables::sstable& sst, const sstables::key& key, uint64_t partition_size, uint64_t rows_count) {
    assert(running());
    std::pair<bool, bool> record{
        __builtin_expect(partition_size > _partition_threshold_bytes, false),
        __builtin_expect(rows_count > _rows_count_threshold, false)};
    if (record.first) {
        ++_stats.partitions_bigger_than_threshold;
    }
    if (record != std::pair{false, false}) {
        return with_sem([&sst, &key, partition_size, rows_count, this] {
            return record_large_partitions(sst, key, partition_size, rows_count);
        }).then([record] {
            return record;
        });
    }
    return make_ready_future<std::pair<bool, bool>>(false, false);
}
...
future<> large_data_handler::maybe_delete_large_data_entries(sstables::shared_sstable sst) {
    ...
    using dt = sstables::large_data_type;

    auto above_threshold = [sst] (dt type) -> bool {
        auto entry = sst->get_large_data_stat(type);
        return entry && entry->above_threshold;
    };

    future<> large_partitions = make_ready_future<>();
    if (above_threshold(dt::partition_size) || above_threshold(dt::rows_in_partition)) {
    ...
    if (above_threshold(dt::row_size)) {
    ...
    if (above_threshold(dt::cell_size)) {
...
static future<> try_record(std::string_view large_table, std::string_view column, const sstables::sstable& sst,  const sstables::key& partition_key, int64_t size,
    ...
    const sstring req = format("INSERT INTO system.large_{}s (keyspace_name, table_name, sstable_name, {}, partition_key, compaction_time{}) VALUES (?, ?, ?, ?, ?, ?{}) USING TTL 2592000",
            large_table, column, extra_fields_str, extra_values);
...
future<> cql_table_large_data_handler::record_large_partitions(const sstables::sstable& sst, const sstables::key& key, uint64_t partition_size, uint64_t rows_count) const {
    return try_record("partition", "partition_size", sst, key, int64_t(partition_size), "partition", "", {"row_count"}, data_value((int64_t)rows_count));
}
...
future<> cql_table_large_data_handler::record_large_cells(const sstables::sstable& sst, const sstables::key& partition_key,
    ...
    if (clustering_key) {
        ...
        return try_record("cell", "cell_size", sst, partition_key, int64_t(cell_size), cell_type, format("{} {}", ck_str, column_name), extra_fields, ck_str, column_name);
    } else {
        return try_record("cell", "cell_size", sst, partition_key, int64_t(cell_size), cell_type, column_name, extra_fields, data_value::make_null(utf8_type), column_name);
    }
}
...
future<> cql_table_large_data_handler::record_large_rows(const sstables::sstable& sst, const sstables::key& partition_key,
    ...
    if (clustering_key) {
        ...
        return try_record("row", "row_size", sst, partition_key, int64_t(row_size), "row", ck_str, extra_fields,  ck_str);
    } else {
        return try_record("row", "row_size", sst, partition_key, int64_t(row_size), "static row", "", extra_fields, data_value::make_null(utf8_type));
    }
}
