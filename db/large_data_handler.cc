...
future<> large_data_handler::maybe_delete_large_data_entries(sstables::shared_sstable sst) {
    ...
    const auto ldts = {ldt::partition_size, ldt::rows_in_partition, ldt::row_size, ldt::cell_size};
    if (std::any_of(ldts.begin(), ldts.end(),
                    [sst] (auto ldt) {
                        auto entry = sst->get_large_data_stat(ldt);
                        return entry && entry->above_threshold;
                    })) {
        return delete_large_data_entries(*schema, std::move(filename));
    }
    return make_ready_future<>();
}

template <typename... Args>
static future<> try_record(std::string_view metric, const sstables::sstable& sst, const sstables::key& partition_key, int64_t value, int64_t threshold, std::string_view desc, std::string_view extra_path, const std::vector<sstring> &extra_fields, Args&&... args) {
    ...
    const sstring req = format("INSERT INTO system.large_data (keyspace_name, table_name, sstable_name, ratio, metric, partition_key, value, compaction_time{}) VALUES (?, ?, ?, ?, ?, ?, ?, ?{}) USING TTL 2592000",
                               extra_fields_str, extra_values);
    float ratio = (threshold != 0 ? static_cast<float>(value) / threshold : std::numeric_limits<float>::infinity());
    ...
    large_data_logger.warn("Writing large {} {}/{}: {}{} ({} bytes) to {}", desc, ks_name, cf_name, pk_str, extra_path, value, sstable_name);
    try {
        co_await db::qctx->execute_cql(req, ks_name, cf_name, sstable_name, ratio, metric, pk_str, value, timestamp, args...);
    } catch (const std::exception& e) {
        large_data_logger.warn("Failed to add a record to system.large_data: ks = {}, table = {}, sst = {} metric = {} exception = {}",
                               ks_name, cf_name, sstable_name, metric, e);
    }
}
...
future<> cql_table_large_data_handler::record_large_cells(const sstables::sstable& sst, const sstables::key& partition_key,
    ...
    if (clustering_key) {
        ...
        return try_record("cell_size", sst, partition_key, int64_t(cell_size), get_cell_threshold_bytes(), cell_type, format("{} {}", ck_str, column_name), extra_fields, ck_str, column_name);
    } else {
        return try_record("cell_size", sst, partition_key, int64_t(cell_size), get_cell_threshold_bytes(), cell_type, column_name, extra_fields, "", column_name);
    }
}
...
future<> cql_table_large_data_handler::record_large_rows(const sstables::sstable& sst, const sstables::key& partition_key,
    ...
    if (clustering_key) {
        ...
        return try_record("row_size", sst, partition_key, int64_t(row_size), get_row_threshold_bytes(), "row", ck_str, extra_fields, ck_str, "");
    } else {
        return try_record("row_size", sst, partition_key, int64_t(row_size), get_row_threshold_bytes(), "static row", "", extra_fields, "", "");
    }
}

future<> cql_table_large_data_handler::delete_large_data_entries(const schema& s, sstring sstable_name) const {
    const sstring req = "DELETE FROM system.large_data WHERE keyspace_name = ? AND table_name = ? AND sstable_name = ?";
    return db::qctx->execute_cql(req, s.ks_name(), s.cf_name(), sstable_name)
            .discard_result()
            .handle_exception([&s, sstable_name] (std::exception_ptr ep) {
                large_data_logger.warn("Failed to drop entries from system.large_data: ks = {}, table = {}, sst = {} exception = {}",
                        s.ks_name(), s.cf_name(), sstable_name, ep);
            });
}
}
