    ...
    for_each_schema_change([&] (schema_ptr base, const std::vector<mutation>& base_mutations,
        ...
        for (auto version : writable_sstable_versions) {
            ...
            if (it == cache.end()) {
                ...
                created_with_base_schema = make_sstable_easy(env, dir.path(), mt, env.manager().configure_writer(), generation_from_value(gen), version, base_mutations.size());

                created_with_changed_schema = env.make_sstable(changed, dir.path().string(), sstables::generation_from_value(gen), version, sstables::sstable::format_types::big);
