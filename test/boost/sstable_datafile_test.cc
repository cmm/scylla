...
static const generation_type gen1{1};
static const generation_type gen2{2};
...
SEASTAR_TEST_CASE(datafile_generation_09) {
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(9), sstables::get_highest_sstable_version(), big);

        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, mt, sst, s, tmpdir_path] {
            auto sst2 = env.make_sstable(s, tmpdir_path, generation_from_value(9), sstables::get_highest_sstable_version(), big);
...
SEASTAR_TEST_CASE(datafile_generation_11) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(11), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, sst, mt, verifier, tomb, &static_set_col, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(11)).then([&env, s, verifier, tomb, &static_set_col] (auto sstp) mutable {
...
SEASTAR_TEST_CASE(datafile_generation_12) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(12), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, tomb, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(12)).then([&env, s, tomb] (auto sstp) mutable {
...
static future<> sstable_compression_test(compressor_ptr c, generation_type generation) {
...
SEASTAR_TEST_CASE(datafile_generation_13) {
    return sstable_compression_test(compressor::lz4, generation_from_value(13));
}

SEASTAR_TEST_CASE(datafile_generation_14) {
    return sstable_compression_test(compressor::snappy, generation_from_value(14));
}

SEASTAR_TEST_CASE(datafile_generation_15) {
    return sstable_compression_test(compressor::deflate, generation_from_value(15));
}
...
SEASTAR_TEST_CASE(datafile_generation_16) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(16), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mtp, sst).then([&env, s, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(16)).then([] (auto s) {
...
SEASTAR_TEST_CASE(datafile_generation_37) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(37), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mtp, sst).then([&env, s, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(37)).then([&env, s, tmpdir_path] (auto sstp) {
...
SEASTAR_TEST_CASE(datafile_generation_38) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(38), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mtp, sst).then([&env, s, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(38)).then([&env, s] (auto sstp) {
                return do_with(dht::partition_range::make_singular(make_dkey(s, "key1")), [&env, sstp, s] (auto& pr) {
                    auto rd = make_lw_shared<flat_mutation_reader_v2>(sstp->make_reader(s, env.make_reader_permit(), pr, s->full_slice()));
                    return read_mutation_from_flat_mutation_reader(*rd).then([sstp, s, rd] (auto mutation) {
                        auto& mp = mutation->partition();
...
SEASTAR_TEST_CASE(datafile_generation_39) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(39), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mtp, sst).then([&env, s, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(39)).then([&env, s] (auto sstp) {
                return do_with(dht::partition_range::make_singular(make_dkey(s, "key1")), [&env, sstp, s] (auto& pr) {
                    auto rd = make_lw_shared<flat_mutation_reader_v2>(sstp->make_reader(s, env.make_reader_permit(), pr, s->full_slice()));
                    return read_mutation_from_flat_mutation_reader(*rd).then([sstp, s, rd] (auto mutation) {
                        auto& mp = mutation->partition();
...
SEASTAR_TEST_CASE(datafile_generation_41) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(41), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, tomb, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(41)).then([&env, s, tomb] (auto sstp) mutable {
                return do_with(dht::partition_range::make_singular(make_dkey(s, "key1")), [&env, sstp, s, tomb] (auto& pr) {
                    auto rd = make_lw_shared<flat_mutation_reader_v2>(sstp->make_reader(s, env.make_reader_permit(), pr, s->full_slice()));
                    return read_mutation_from_flat_mutation_reader(*rd).then([sstp, s, tomb, rd] (auto mutation) {
                        auto& mp = mutation->partition();
...
SEASTAR_TEST_CASE(datafile_generation_47) {
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(47), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation_from_value(47)).then([&env, s] (auto sstp) mutable {
...
SEASTAR_TEST_CASE(test_counter_write) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        return seastar::async([&env, tmpdir_path] {
            ...
            auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(900), sstables::get_highest_sstable_version(), big);
            write_memtable_to_sstable_for_test(*mt, sst).get();

            auto sstp = env.reusable_sst(s, tmpdir_path, generation_from_value(900)).get0();
...
SEASTAR_TEST_CASE(check_read_indexes) {
  return test_env::do_with_async([] (test_env& env) {
      for_each_sstable_version([&env] (const sstables::sstable::version_types version) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("summary_test", s), gen1, version, big);
...
SEASTAR_TEST_CASE(check_multi_schema) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        for_each_sstable_version([&env] (const sstables::sstable::version_types version) {
            ...
            auto sst = env.make_sstable(s, get_test_dir("multi_schema_test", s), gen1, version, big);
            auto f = sst->load();
...
SEASTAR_TEST_CASE(test_sliced_mutation_reads) {
    ...
    return test_env::do_with_async([] (test_env& env) {
      for (auto version : all_sstable_versions) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("sliced_mutation_reads", s), gen1, version, big);
        sst->load().get0();
...
SEASTAR_TEST_CASE(test_wrong_range_tombstone_order) {
    ...
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : all_sstable_versions) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("wrong_range_tombstone_order", s), gen1, version, big);
        sst->load().get0();
...
SEASTAR_TEST_CASE(test_counter_read) {
        ...
        return test_env::do_with_async([] (test_env& env) {
          for (const auto version : all_sstable_versions) {
            ...
            auto sst = env.make_sstable(s, get_test_dir("counter_test", s), generation_from_value(5), version, big);
            sst->load().get();
            auto reader = sstable_reader_v2(sst, s, env.make_reader_permit());
            auto close_reader = deferred_close(reader);
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        return seastar::async([&env, tmpdir_path] {
            for (const auto version : writable_sstable_versions) {
                ...
                auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(53), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                auto sstp = env.reusable_sst(s, tmpdir_path, generation_from_value(53), version).get0();
    ...
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : all_sstable_versions) {
        auto s = schema_builder("ks", "promoted_index_read")
                ...
                .build();
        auto sst = env.make_sstable(s, get_test_dir("promoted_index_read", s), gen1, version, big);
        sst->load().get0();
...
static void test_min_max_clustering_key(test_env& env, schema_ptr s, std::vector<bytes> exploded_pk, std::vector<std::vector<bytes>> exploded_cks,
    ...
    auto tmp = tmpdir();
    auto sst = env.make_sstable(s, tmp.path().string(), gen1, version, big);
    write_memtable_to_sstable_for_test(*mt, sst).get();
    sst = env.reusable_sst(s, tmp.path().string(), gen1, version).get0();
    ...
    return test_env::do_with_async([] (test_env& env) {
        for (const auto version : writable_sstable_versions) {
...

            {
                ...
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), gen2, version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), gen2, version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                check_min_max_column_names(sst, {"c1"}, {"c1"});
            }

            {
                ...
                m.set_clustered_cell(c_key, r1_col, make_atomic_cell(int32_type, int32_type->decompose(1)));
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(3), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(3), version).get0();
                BOOST_REQUIRE(!sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                check_min_max_column_names(sst, {"c1"}, {"c1"});
            }

            {
                ...
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(4), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(4), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                check_min_max_column_names(sst, {"c1"}, {"c1"});
            }

            {
                ...
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(5), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(5), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
...

            {
                ...
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(6), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(6), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                if (version >= sstable_version_types::mc) {
...

            {
                ...
                m.partition().apply_delete(*s, std::move(rt));
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(7), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(7), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                if (version >= sstable_version_types::mc) {
...

            {
                ...
                m.partition().apply_delete(*s, std::move(rt));
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(8), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(8), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                if (version >= sstable_version_types::mc) {
...

            {
                ...
                m.partition().apply_delete(*s, std::move(rt));
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(9), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(9), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                if (version >= sstable_version_types::mc) {
            ...
            if (version >= sstable_version_types::mc) {
                {
                    ...
                    m.partition().apply_delete(*s, std::move(rt));
                    mt->apply(std::move(m));
                    auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(10), version, big);
                    write_memtable_to_sstable_for_test(*mt, sst).get();
                    sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(10), version).get0();
                    BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
...

                {
                    ...
                    m.partition().apply_delete(*s, std::move(rt));
                    mt->apply(std::move(m));
                    auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(11), version, big);
                    write_memtable_to_sstable_for_test(*mt, sst).get();
                    sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(11), version).get0();
                    BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
...

                {
                    ...
                    mt->apply(std::move(m));
                    auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(12), version, big);
                    write_memtable_to_sstable_for_test(*mt, sst).get();
                    sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(12), version).get0();
                    BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                    check_min_max_column_names(sst, {}, {});
                }
            }
    ...
    return test_env::do_with_async([] (test_env& env) {
        for (const auto version : writable_sstable_versions) {
...

            {
                ...
                m.set_clustered_cell(c_key, r1_col, make_dead_atomic_cell(3600));
                mt->apply(std::move(m));
                auto sst = env.make_sstable(s, tmp.path().string(), generation_from_value(2), version, big);
                write_memtable_to_sstable_for_test(*mt, sst).get();
                sst = env.reusable_sst(s, tmp.path().string(), generation_from_value(2), version).get0();
                BOOST_REQUIRE(sst->get_stats_metadata().estimated_tombstone_drop_time.bin.size());
                check_min_max_column_names(sst, {"c1", "c2"}, {"c1", "c2"});
            }
...
SEASTAR_TEST_CASE(test_partition_skipping) {
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : all_sstable_versions) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("partition_skipping",s), gen1, version, big);
        sst->load().get0();
...
SEASTAR_TEST_CASE(test_repeated_tombstone_skipping) {
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : writable_sstable_versions) {
        ...
        auto sst = make_sstable_easy(env, dir.path(), make_flat_mutation_reader_from_mutations_v2(table.schema(), std::move(permit), { std::move(mut) }), cfg, gen1, version);
...
SEASTAR_TEST_CASE(test_skipping_using_index) {
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : writable_sstable_versions) {
        ...
        auto sst = make_sstable_easy(env, dir.path(), make_flat_mutation_reader_from_mutations_v2(table.schema(), env.make_reader_permit(), partitions), cfg, gen1, version);
...
SEASTAR_TEST_CASE(test_unknown_component) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sstp = env.reusable_sst(uncompressed_schema(), tmp.path().string() + "/unknown_component", gen1).get0();
        ...
        sstp = env.reusable_sst(uncompressed_schema(), tmp.path().string(), gen1).get0();
        sstp->set_generation(gen2).get();
        BOOST_REQUIRE(!file_exists(tmp.path().string() +  "/la-1-big-UNKNOWN.txt").get0());
...
SEASTAR_TEST_CASE(sstable_set_incremental_selector) {
  return test_env::do_with([] (test_env& env) {
...

    {
        ...
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(1), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(2), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(3), key_and_token_pair[3].first, key_and_token_pair[4].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(4), key_and_token_pair[4].first, key_and_token_pair[4].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(5), key_and_token_pair[4].first, key_and_token_pair[5].first, 1));
    ...
    {
        sstable_set set = cs.make_sstable_set(s);
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(0), key_and_token_pair[0].first, key_and_token_pair[1].first, 0));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(1), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(2), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(3), key_and_token_pair[3].first, key_and_token_pair[4].first, 1));
        set.insert(sstable_for_overlapping_test(env, s, generation_from_value(4), key_and_token_pair[4].first, key_and_token_pair[4].first, 1));
...
SEASTAR_TEST_CASE(sstable_set_erase) {
  return test_env::do_with([] (test_env& env) {
    ...
    // check that sstable_set::erase is capable of working properly when a non-existing element is given.
    {
        ...
        auto sst = sstable_for_overlapping_test(env, s, generation_from_value(0), key_and_token_pair[0].first, key_and_token_pair[0].first, 0);
        set.insert(sst);
        BOOST_REQUIRE(set.all()->size() == 1);

        auto unleveled_sst = sstable_for_overlapping_test(env, s, generation_from_value(1), key_and_token_pair[0].first, key_and_token_pair[0].first, 0);
        auto leveled_sst = sstable_for_overlapping_test(env, s, generation_from_value(2), key_and_token_pair[0].first, key_and_token_pair[0].first, 1);
...

    {
        ...
        {
            auto sst = sstable_for_overlapping_test(env, s, generation_from_value(0), key_and_token_pair[0].first, key_and_token_pair[0].first, 1);
            set.insert(sst);
            BOOST_REQUIRE(set.all()->size() == 1);
        }

        auto sst2 = sstable_for_overlapping_test(env, s, generation_from_value(0), key_and_token_pair[0].first, key_and_token_pair[0].first, 1);
        set.insert(sst2);
...

    {
        ...
        auto sst2 = sstable_for_overlapping_test(env, s, generation_from_value(1), key_and_token_pair[0].first, key_and_token_pair[0].first, 0);
        set.erase(sst2);
        BOOST_REQUIRE(set.all()->size() == 1);
        BOOST_REQUIRE(set.all()->contains(sst));
    }
...
SEASTAR_TEST_CASE(sstable_tombstone_histogram_test) {
    return test_env::do_with_async([] (test_env& env) {
        for (auto version : writable_sstable_versions) {
            ...
            auto tmp = tmpdir();
            auto sst_gen = [&env, s, &tmp, gen = make_lw_shared<generation_value_type>(1), version]() mutable {
...
SEASTAR_TEST_CASE(sstable_bad_tombstone_histogram_test) {
    return test_env::do_with_async([] (test_env& env) {
        auto builder = schema_builder("tests", "tombstone_histogram_test")
                ...
                .with_column("value", int32_type);
        auto s = builder.build();
        auto sst = env.reusable_sst(s, "test/resource/sstables/bad_tombstone_histogram", gen1).get0();
        auto histogram = sst->get_stats_metadata().estimated_tombstone_drop_time;
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto make_shared_sstable = [&] (std::unordered_set<unsigned> shards, unsigned ignore_msb, unsigned smp_count) {
            ...
            auto sst_gen = [&env, s, &tmp, gen, ignore_msb] () mutable {
                ...
                auto sst = env.make_sstable(std::move(schema), tmp.path().string(), generation_from_value((*gen)++), sstables::get_highest_sstable_version(), big);
                return sst;
            };
...
SEASTAR_TEST_CASE(test_wrong_counter_shard_order) {
        ...
        return test_env::do_with_async([] (test_env& env) {
          for (const auto version : all_sstable_versions) {
            ...
            auto sst = env.make_sstable(s, get_test_dir("wrong_counter_shard_order", s), gen2, version, big);
            sst->load().get0();
            auto reader = sstable_reader_v2(sst, s, env.make_reader_permit());
            auto close_reader = deferred_close(reader);
...
SEASTAR_TEST_CASE(test_broken_promoted_index_is_skipped) {
    ...
    return test_env::do_with_async([] (test_env& env) {
      for (const auto version : all_sstable_versions) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("broken_non_compound_pi_and_range_tombstone", s), gen1, version, big);
        sst->load().get0();
...
SEASTAR_TEST_CASE(summary_rebuild_sanity) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        sst = env.reusable_sst(s, tmp.path().string(), gen1).get0();
...
SEASTAR_TEST_CASE(test_reads_cassandra_static_compact) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst = env.make_sstable(s, get_test_dir("cassandra_static_compact", s), gen1, sstables::sstable::version_types::mc, big);
        sst->load().get0();
...
SEASTAR_TEST_CASE(test_zero_estimated_partitions) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        for (const auto version : writable_sstable_versions) {
            ...
            sstable_writer_config cfg = env.manager().configure_writer();
            auto sst = make_sstable_easy(env, tmp, std::move(mr), cfg, generation_from_value(0), version, 0);
...
SEASTAR_TEST_CASE(test_missing_partition_end_fragment) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        for (const auto version : writable_sstable_versions) {
            ...
            auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(0), version, big);
            sstable_writer_config cfg = env.manager().configure_writer();
...
SEASTAR_TEST_CASE(test_sstable_origin) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        for (const auto version : all_sstable_versions) {
            ...
            sstable_writer_config cfg = env.manager().configure_writer("");
            auto sst = make_sstable_easy(env, tmp, std::move(mr), cfg, generation_from_value(gen++), version, 0);
            ...
            sst = make_sstable_easy(env, tmp, std::move(mr), cfg, generation_from_value(gen++), version, 0);
...
SEASTAR_TEST_CASE(compound_sstable_set_basic_test) {
    return test_env::do_with([] (test_env& env) {
        ...
        set1->insert(sstable_for_overlapping_test(env, s, generation_from_value(1), key_and_token_pair[0].first, key_and_token_pair[1].first, 0));
        set2->insert(sstable_for_overlapping_test(env, s, generation_from_value(2), key_and_token_pair[0].first, key_and_token_pair[1].first, 0));
        set2->insert(sstable_for_overlapping_test(env, s, generation_from_value(3), key_and_token_pair[0].first, key_and_token_pair[1].first, 0));

        BOOST_REQUIRE(boost::accumulate(*compound->all() | boost::adaptors::transformed([] (const sstables::shared_sstable& sst) { return generation_value(sst->generation()); }), generation_value_type(0)) == 6);
        {
            unsigned found = 0;
...
SEASTAR_TEST_CASE(sstable_reader_with_timeout) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        return async([&env, tmpdir_path] {
            ...
            auto sst = env.make_sstable(s, tmpdir_path, generation_from_value(12), sstables::get_highest_sstable_version(), big);
            write_memtable_to_sstable_for_test(*mt, sst).get();
            auto sstp = env.reusable_sst(s, tmpdir_path, generation_from_value(12)).get0();
    ...
    return test_setup::do_with_tmp_directory([&] (test_env& env, sstring tmpdir_path) {
        ...
        for (const auto version : writable_sstable_versions) {
            ...
            for (const auto& compression_params : {no_compression_params, lz4_compression_params}) {
                ...
                auto sst = env.make_sstable(sst_schema, tmpdir_path, generation_from_value(gen++), version, big);
                sstable_writer_config cfg = env.manager().configure_writer();
...
SEASTAR_TEST_CASE(partial_sstable_deletion_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst_gen = [&env, s, &tmp] () {
            return env.make_sstable(s, tmp.path().string(), gen1, sstables::get_highest_sstable_version(), big);
        };
...
SEASTAR_TEST_CASE(test_index_fast_forwarding_after_eof) {
    return test_setup::do_with_tmp_directory([&] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(schema, tmpdir_path, generation_from_value(0), writable_sstable_versions.back(), big);
        {
            auto mr = make_flat_mutation_reader_from_mutations_v2(schema, permit, muts);
            auto close_mr = deferred_close(mr);
