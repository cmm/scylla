...
static future<sstables::shared_sstable> open_sstable(test_env& env, schema_ptr schema, sstring dir, generation::type generation) {
...
static future<std::vector<sstables::shared_sstable>> open_sstables(test_env& env, schema_ptr s, sstring dir, const std::vector<generation::value_type>& generations) {
    return do_with(std::vector<sstables::shared_sstable>(),
            [&env, dir = std::move(dir), generations = std::move(generations), s] (auto& ret) mutable {
                return parallel_for_each(generations, [&env, &ret, &dir, s] (generation::value_type generation) {
            return open_sstable(env, s, dir, generation::from_value(generation)).then([&ret] (sstables::shared_sstable sst) {
...
SEASTAR_TEST_CASE(compact) {
  return sstables::test_env::do_with([] (sstables::test_env& env) {
    BOOST_REQUIRE(smp::count == 1);
    constexpr generation::value_type generation = 17;
    ...
    return test_setup::do_with_tmp_directory([s, generation, cf, cm] (test_env& env, sstring tmpdir_path) {
        return open_sstables(env, s, "test/resource/sstables/compaction", {1,2,3}).then([&env, tmpdir_path, s, cf, cm, generation] (auto sstables) {
            auto new_sstable = [&env, gen = make_lw_shared<generation::value_type>(generation), s, tmpdir_path] {
                return env.make_sstable(s, tmpdir_path,
                        generation::from_value((*gen)++), sstables::get_highest_sstable_version(), sstables::sstable::format_types::big);
            };
            return compact_sstables(*cm, sstables::compaction_descriptor(std::move(sstables), default_priority_class()), *cf, new_sstable).then([&env, s, generation, cf, cm, tmpdir_path] (auto) {
                ...
                return open_sstable(env, s, tmpdir_path, generation::from_value(generation)).then([&env, s] (shared_sstable sst) {
...
static future<std::vector<generation::value_type>> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<generation::value_type> generations_to_compact,
    ...
    auto created = make_lw_shared<std::vector<generation::value_type>>();
    ...
    return f.then([&env, generations, sstables, s, create_sstables, min_sstable_size, tmpdir_path] () mutable {
        ...
        return do_for_each(*generations, [&env, generations, sstables, s, min_sstable_size, tmpdir_path] (generation::value_type generation) {
            auto mt = make_lw_shared<replica::memtable>(s);
            ...
            auto sst = env.make_sstable(s, tmpdir_path, generation::from_value(generation), sstables::get_highest_sstable_version(), big);
...
static future<> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<generation::value_type> generations_to_compact, generation::value_type new_generation, bool create_sstables = true) {
...
static future<> check_compacted_sstables(test_env& env, sstring tmpdir_path, generation::value_type generation, std::vector<generation::value_type> compacted_generations) {
    auto s = make_shared_schema({}, some_keyspace, some_column_family,
        {{"p1", utf8_type}}, {{"c1", utf8_type}}, {{"r1", utf8_type}}, {}, utf8_type);

    auto generations = make_lw_shared<decltype(compacted_generations)>(std::move(compacted_generations));

    return open_sstable(env, s, tmpdir_path, generation::from_value(generation)).then([&env, s, generations] (shared_sstable sst) {
        ...
        return with_closeable(std::move(reader), [generations, s, keys] (flat_mutation_reader_v2& reader) {
            return do_for_each(*generations, [&reader, keys] (generation::value_type generation) mutable {
...
static void add_sstable_for_leveled_test(test_env& env, lw_shared_ptr<replica::column_family> cf, generation::type gen, uint64_t fake_data_size,
...
static shared_sstable add_sstable_for_overlapping_test(test_env& env, lw_shared_ptr<replica::column_family> cf, generation::type gen, sstring first_key, sstring last_key, stats_metadata stats = {}) {
    auto sst = env.make_sstable(cf->schema(), "", gen, la, big);
    sstables::test(sst).set_values(std::move(first_key), std::move(last_key), std::move(stats));
    column_family_test(cf).add_sstable(sst);
    return sst;
}
static shared_sstable sstable_for_overlapping_test(test_env& env, const schema_ptr& schema, generation::type gen, sstring first_key, sstring last_key, uint32_t level = 0) {
...
static shared_sstable get_sstable(const lw_shared_ptr<replica::column_family>& cf, generation::type generation) {
...
static bool sstable_overlaps(const lw_shared_ptr<replica::column_family>& cf, generation::type gen1, generation::type gen2) {
...
SEASTAR_TEST_CASE(leveled_01) {
  ...
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), max_sstable_size, /*level*/0, min_key, max_key);
    BOOST_REQUIRE(cf->get_sstables()->size() == 1);

    add_sstable_for_leveled_test(env, cf, generation::from_value(2), max_sstable_size, /*level*/0, key_and_token_pair[1].first, max_key);
    ...
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(2)) == true);
...
SEASTAR_TEST_CASE(leveled_02) {
  ...
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), max_sstable_size, /*level*/0, min_key, key_and_token_pair[10].first);
    BOOST_REQUIRE(cf->get_sstables()->size() == 1);

    add_sstable_for_leveled_test(env, cf, generation::from_value(2), max_sstable_size, /*level*/0, min_key, key_and_token_pair[20].first);
    BOOST_REQUIRE(cf->get_sstables()->size() == 2);

    add_sstable_for_leveled_test(env, cf, generation::from_value(3), max_sstable_size, /*level*/0, key_and_token_pair[30].first, max_key);
    ...
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(2)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(1)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(3)) == false);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(3)) == false);
...
SEASTAR_TEST_CASE(leveled_03) {
  ...
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), /*data_size*/1024*1024, /*level*/0, min_key, key_and_token_pair[10].first);
    add_sstable_for_leveled_test(env, cf, generation::from_value(2), /*data_size*/1024*1024, /*level*/0, min_key, key_and_token_pair[20].first);
    // Creating a sstable of level 1 which overlap with two sstables above.
    add_sstable_for_leveled_test(env, cf, generation::from_value(3), /*data_size*/1024*1024, /*level*/1, min_key, key_and_token_pair[30].first);
    // Creating a sstable of level 1 which doesn't overlap with any sstable.
    add_sstable_for_leveled_test(env, cf, generation::from_value(4), /*data_size*/1024*1024, /*level*/1, key_and_token_pair[40].first, max_key);
    ...
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(2)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(3)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(3)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(4)) == false);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(4)) == false);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(3), generation::from_value(4)) == false);
...
SEASTAR_TEST_CASE(leveled_04) {
  ...
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), /*data_size*/max_sstable_size_in_bytes, /*level*/0, min_key, max_key);
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(2), /*data_size*/max_bytes_for_l1, /*level*/1, min_key, key_and_token_pair[25].first);
    add_sstable_for_leveled_test(env, cf, generation::from_value(3), /*data_size*/max_bytes_for_l1, /*level*/1, key_and_token_pair[26].first, max_key);
    ...
    // level2.
    add_sstable_for_leveled_test(env, cf, generation::from_value(4), /*data_size*/max_sstable_size_in_bytes, /*level*/2, min_key, max_key);
    ...
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(1), generation::from_value(3)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(3)) == false);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(3), generation::from_value(4)) == true);
    BOOST_REQUIRE(sstable_overlaps(cf, generation::from_value(2), generation::from_value(4)) == true);
        ...
        return compact_sstables(env, tmpdir_path, { 48, 49 }, 50, true, 1024*1024, compaction_strategy_type::leveled).then([tmpdir_path] (auto generations) {
            ...
            return seastar::async([&, generations = std::move(generations), tmpdir_path] {
                for (auto gen : generations) {
                    auto fname = sstable::filename(tmpdir_path, "ks", "cf", sstables::get_highest_sstable_version(), generation::from_value(gen), big, component_type::Data);
...
SEASTAR_TEST_CASE(leveled_06) {
  ...
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), /*data_size*/max_bytes_for_l1*2, /*level*/1, "a", "a");
    BOOST_REQUIRE(cf->get_sstables()->size() == 1);
...
SEASTAR_TEST_CASE(leveled_07) {
  return test_env::do_with([] (test_env& env) {
    ...
    for (auto i = 0; i < leveled_manifest::MAX_COMPACTING_L0*2; i++) {
        add_sstable_for_leveled_test(env, cf, generation::from_value(i), 1024*1024, /*level*/0, "a", "a", i /* max timestamp */);
    }
    auto candidates = get_candidates_for_leveled_strategy(*cf);
    sstables::size_tiered_compaction_strategy_options stcs_options;
    auto table_s = make_table_state_for_test(cf, env);
...
SEASTAR_TEST_CASE(leveled_invariant_fix) {
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(0), sstable_max_size, /*level*/1, min_key, min_key);

    for (auto i = 1; i < sstables_no-1; i++) {
        add_sstable_for_leveled_test(env, cf, generation::from_value(i), sstable_max_size, /*level*/1, key_and_token_pair[i].first, key_and_token_pair[i].first);
    }
    // add large token span sstable into level 1, which overlaps with all sstables added in loop above.
    add_sstable_for_leveled_test(env, cf, generation::from_value(sstables_no), sstable_max_size, 1, key_and_token_pair[1].first, max_key);
...
SEASTAR_TEST_CASE(leveled_stcs_on_L0) {
  return test_env::do_with([] (test_env& env) {
    ...
    add_sstable_for_leveled_test(env, cf, generation::from_value(0), sstable_max_size_in_mb*1024*1024, /*level*/1, key_and_token_pair[0].first, key_and_token_pair[0].first);
    for (auto gen = 0; gen < l0_sstables_no; gen++) {
        add_sstable_for_leveled_test(env, cf, generation::from_value(gen+1), l0_sstables_size, /*level*/0, key_and_token_pair[0].first, key_and_token_pair[0].first);
    }
    auto candidates = get_candidates_for_leveled_strategy(*cf);
...
SEASTAR_TEST_CASE(overlapping_starved_sstables_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto max_bytes_for_l1 = leveled_manifest::max_bytes_for_level(1, max_sstable_size_in_bytes);
    add_sstable_for_leveled_test(env, cf, generation::from_value(1), max_bytes_for_l1*1.1, /*level*/1, min_key, key_and_token_pair[2].first);
    add_sstable_for_leveled_test(env, cf, generation::from_value(2), max_sstable_size_in_bytes, /*level*/2, min_key, key_and_token_pair[1].first);
    add_sstable_for_leveled_test(env, cf, generation::from_value(3), max_sstable_size_in_bytes, /*level*/3, min_key, key_and_token_pair[1].first);
...
SEASTAR_TEST_CASE(check_overlapping) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto sst1 = add_sstable_for_overlapping_test(env, cf, generation::from_value(1), min_key, key_and_token_pair[1].first);
    auto sst2 = add_sstable_for_overlapping_test(env, cf, generation::from_value(2), min_key, key_and_token_pair[2].first);
    auto sst3 = add_sstable_for_overlapping_test(env, cf, generation::from_value(3), key_and_token_pair[3].first, max_key);
    auto sst4 = add_sstable_for_overlapping_test(env, cf, generation::from_value(4), min_key, max_key);
    BOOST_REQUIRE(cf->get_sstables()->size() == 4);
...
SEASTAR_TEST_CASE(tombstone_purge_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst_gen = [&env, s, &tmp, gen = make_lw_shared<unsigned>(1)] () mutable {
            return env.make_sstable(s, tmp.path().string(), generation::from_value((*gen)++), sstables::get_highest_sstable_version(), big);
        };
...
SEASTAR_TEST_CASE(sstable_rewrite) {
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst = env.make_sstable(s, tmpdir_path, generation::from_value(51), sstables::get_highest_sstable_version(), big);
        return write_memtable_to_sstable_for_test(*mt, sst).then([&env, s, sst, tmpdir_path] {
            return env.reusable_sst(s, tmpdir_path, generation::from_value(51));
            ...
            auto creator = [&env, new_tables, s, tmpdir_path] {
                auto sst = env.make_sstable(s, tmpdir_path, generation::from_value(52), sstables::get_highest_sstable_version(), big);
    ...
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        return seastar::async([&env, tmpdir_path] {
            for (auto version : writable_sstable_versions) {
                ...
                auto get_usable_sst = [&env, s, tmpdir_path, version](replica::memtable &mt, generation::value_type gen) -> future<sstable_ptr> {
                    auto sst = env.make_sstable(s, tmpdir_path, generation::from_value(gen), version, big);
                    return write_memtable_to_sstable_for_test(mt, sst).then([&env, sst, gen, s, tmpdir_path, version] {
                        return env.reusable_sst(s, tmpdir_path, generation::from_value(gen), version);
                    });
                };
                ...
                auto creator = [&env, s, tmpdir_path, version, gen = make_lw_shared<unsigned>(56)] { return env.make_sstable(s, tmpdir_path, generation::from_value((*gen)++), version, big); };
...
SEASTAR_TEST_CASE(get_fully_expired_sstables_test) {
  return test_env::do_with_async([] (test_env& env) {
...

    {
        ...
        auto sst1 = add_sstable_for_overlapping_test(env, cf, generation::from_value(1), min_key, key_and_token_pair[1].first, build_stats(t0, t1, t1));
        auto sst2 = add_sstable_for_overlapping_test(env, cf, generation::from_value(2), min_key, key_and_token_pair[2].first, build_stats(t0, t1, std::numeric_limits<int32_t>::max()));
        auto sst3 = add_sstable_for_overlapping_test(env, cf, generation::from_value(3), min_key, max_key, build_stats(t3, t4, std::numeric_limits<int32_t>::max()));
...

    {
        ...
        auto sst1 = add_sstable_for_overlapping_test(env, cf, generation::from_value(1), min_key, key_and_token_pair[1].first, build_stats(t0, t1, t1));
        auto sst2 = add_sstable_for_overlapping_test(env, cf, generation::from_value(2), min_key, key_and_token_pair[2].first, build_stats(t2, t3, std::numeric_limits<int32_t>::max()));
        auto sst3 = add_sstable_for_overlapping_test(env, cf, generation::from_value(3), min_key, max_key, build_stats(t3, t4, std::numeric_limits<int32_t>::max()));
        std::vector<sstables::shared_sstable> compacting = { sst1, sst2 };
...
SEASTAR_TEST_CASE(compaction_with_fully_expired_table) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst = sst_gen();
        write_memtable_to_sstable_for_test(*mt, sst).get();
        sst = env.reusable_sst(s, tmp.path().string(), generation::from_value(1)).get0();
...
SEASTAR_TEST_CASE(basic_date_tiered_strategy_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    for (auto i = 1; i <= min_threshold; i++) {
        auto sst = add_sstable_for_overlapping_test(env, cf, generation::from_value(i), "a", "a",
    ...
    auto sst = add_sstable_for_overlapping_test(env, cf, generation::from_value(min_threshold + 1), "a", "a",
...
SEASTAR_TEST_CASE(date_tiered_strategy_test_2) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto sst2 = add_sstable_for_overlapping_test(env, cf, generation::from_value(min_threshold + 2), "a", "a",
        build_stats(timestamp, timestamp, std::numeric_limits<int32_t>::max()));
    ...
    auto table_s = make_table_state_for_test(cf, env);
    auto sstables = manifest.get_next_sstables(*table_s, candidates, gc_before);
    std::unordered_set<generation::type> gens;
    for (auto sst : sstables) {
        gens.insert(sst->generation());
    }
    BOOST_REQUIRE(sstables.size() == size_t(min_threshold + 1));
    BOOST_REQUIRE(gens.contains(generation::from_value(min_threshold + 1)));
    BOOST_REQUIRE(!gens.contains(generation::from_value(min_threshold + 2)));
...
SEASTAR_TEST_CASE(time_window_strategy_ts_resolution_check) {
  return test_env::do_with([] (test_env& env) {
...

    {
        ...
        auto sst = env.make_sstable(s, "", generation::from_value(1), la, big);
...
SEASTAR_TEST_CASE(min_max_clustering_key_test_2) {
    return test_env::do_with_async([] (test_env& env) {
        for (const auto version : writable_sstable_versions) {
            ...
            auto sst = env.make_sstable(s, tmp.path().string(), generation::from_value(1), version, big);
            write_memtable_to_sstable_for_test(*mt, sst).get();
            sst = env.reusable_sst(s, tmp.path().string(), generation::from_value(1), version).get0();
            ...
            mt->apply(std::move(m));
            auto sst2 = env.make_sstable(s, tmp.path().string(), generation::from_value(2), version, big);
            write_memtable_to_sstable_for_test(*mt, sst2).get();
            sst2 = env.reusable_sst(s, tmp.path().string(), generation::from_value(2), version).get0();
            check_min_max_column_names(sst2, {"9ck101"}, {"9ck298"});

            auto creator = [&env, s, &tmp, version] { return env.make_sstable(s, tmp.path().string(), generation::from_value(3), version, big); };
...
SEASTAR_TEST_CASE(size_tiered_beyond_max_threshold_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    for (auto i = 0; i < (max_threshold+1); i++) { // (max_threshold+1) sstables of similar size
        auto sst = env.make_sstable(cf.schema(), "", generation::from_value(i), la, big);
...
SEASTAR_TEST_CASE(sstable_expired_data_ratio) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst = env.make_sstable(s, tmp.path().string(), generation::from_value(1), sstables::get_highest_sstable_version(), big);
        write_memtable_to_sstable_for_test(*mt, sst).get();
        sst = env.reusable_sst(s, tmp.path().string(), generation::from_value(1)).get0();
        ...
        auto creator = [&, gen = make_lw_shared<unsigned>(2)] {
            auto sst = env.make_sstable(s, tmp.path().string(), generation::from_value((*gen)++), sstables::get_highest_sstable_version(), big);
            return sst;
        };
    ...
    return do_with_cql_env([this] (cql_test_env& cql_env) -> future<> {
        return test_env::do_with_async([this, &cql_env] (test_env& env) {
            ...
            auto sst_gen = [&env, schema, &tmp, gen = make_lw_shared<unsigned>(1)] () mutable {
                return env.make_sstable(schema, tmp.path().string(), generation::from_value((*gen)++));
            };
...
SEASTAR_TEST_CASE(sstable_run_based_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto do_compaction = [&] (size_t expected_input, size_t expected_output) -> std::vector<shared_sstable> {
            ...
            auto sstable_run = boost::copy_range<std::set<generation::type>>(desc.sstables
...
SEASTAR_TEST_CASE(partial_sstable_run_filtered_out_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto generation_exists = [&cf] (generation::type generation) {
            auto sstables = cf->get_sstables();
            auto entry = boost::range::find_if(*sstables, [generation] (shared_sstable sst) { return generation == sst->generation(); });
...
SEASTAR_TEST_CASE(test_bug_6472) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst_gen = [&env, s, tmpdir_path, gen = make_lw_shared<unsigned>(1)] () mutable {
            return env.make_sstable(s, tmpdir_path, generation::from_value((*gen)++), sstables::get_highest_sstable_version(), big);
        };
...
SEASTAR_TEST_CASE(sstable_needs_cleanup_test) {
  return test_env::do_with([] (test_env& env) {
    ...
    auto sst_gen = [&env, s, gen = make_lw_shared<unsigned>(1)] (sstring first, sstring last) mutable {
        return sstable_for_overlapping_test(env, s, generation::from_value((*gen)++), first, last);
    };
...
SEASTAR_TEST_CASE(stcs_reshape_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        for (auto gen = 1; gen <= s->max_compaction_threshold(); gen++) {
            auto sst = env.make_sstable(s, "", generation::from_value(gen));
            sstables::test(sst).set_data_file_size(1);
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        // non overlapping
        {
            ...
            for (auto i = 0; i < 256; i++) {
                auto sst = env.make_sstable(s, "", generation::from_value(i + 1));
        ...
        // single sstable
        {
            auto sst = env.make_sstable(s, "", generation::from_value(1));
            auto key = keys[0].first;
            sstables::test(sst).set_values_for_leveled_strategy(1 /* size */, 0 /* level */, 0 /* max ts */, key, key);
...
SEASTAR_TEST_CASE(twcs_reshape_with_disjoint_set_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto sst_gen = [&env, s, &tmp, gen = make_lw_shared<unsigned>(1)]() {
            return env.make_sstable(s, tmp.path().string(), generation::from_value((*gen)++), sstables::sstable::version_types::md, big);
        };
...

        {
            ...
            std::unordered_set<generation::type> generations_for_small_files;
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto make_table_with_single_fully_expired_sstable = [&] (auto idx) {
            ...
            auto sst_gen = [&env, s, dir = cfg.datadir, gen = make_lw_shared<unsigned>(1)] () mutable {
                return env.make_sstable(s, dir, generation::from_value((*gen)++), sstables::sstable::version_types::md, big);
            };
...
SEASTAR_TEST_CASE(compound_sstable_set_incremental_selector_test) {
    return test_env::do_with([] (test_env& env) {
...

        {
            ...
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(1), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
            set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(2), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(3), key_and_token_pair[3].first, key_and_token_pair[4].first, 1));
            set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(4), key_and_token_pair[4].first, key_and_token_pair[4].first, 1));
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(5), key_and_token_pair[4].first, key_and_token_pair[5].first, 1));
...

        {
            auto set1 = make_lw_shared<sstable_set>(cs.make_sstable_set(s));
            auto set2 = make_lw_shared<sstable_set>(cs.make_sstable_set(s));
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(0), key_and_token_pair[0].first, key_and_token_pair[1].first, 0));
            set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(1), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(2), key_and_token_pair[0].first, key_and_token_pair[1].first, 1));
            set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(3), key_and_token_pair[3].first, key_and_token_pair[4].first, 1));
            set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(4), key_and_token_pair[4].first, key_and_token_pair[4].first, 1));
            set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(5), key_and_token_pair[4].first, key_and_token_pair[5].first, 1));
...

        {
            ...
            auto incremental_selection_test = [&] (strategy_param param) {
                ...
                set1->insert(sstable_for_overlapping_test(env, s, generation::from_value(0), key_and_token_pair[1].first, key_and_token_pair[1].first, 1));
                set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(1), key_and_token_pair[0].first, key_and_token_pair[2].first, 1));
                set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(2), key_and_token_pair[3].first, key_and_token_pair[3].first, 1));
                set2->insert(sstable_for_overlapping_test(env, s, generation::from_value(3), key_and_token_pair[4].first, key_and_token_pair[4].first, 1));
...
SEASTAR_TEST_CASE(simple_backlog_controller_test) {
    auto run_controller_test = [] (sstables::compaction_strategy_type compaction_strategy_type, test_env& env) {
        ...
        auto add_sstable = [&env, &manager, gen = make_lw_shared<unsigned>(1)] (replica::table& t, uint64_t data_size, int level) {
            auto sst = env.make_sstable(t.schema(), "", generation::from_value((*gen)++), la, big);
...
SEASTAR_TEST_CASE(test_compaction_strategy_cleanup_method) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto run_cleanup_strategy_test = [&] (sstables::compaction_strategy_type compaction_strategy_type, size_t per_job_files, auto&&... args) {
            ...
            auto generations = boost::copy_range<std::unordered_set<generation::type>>(candidates | boost::adaptors::transformed(std::mem_fn(&sstables::sstable::generation)));
