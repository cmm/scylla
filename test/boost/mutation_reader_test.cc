...
struct sst_factory {
    ...
    sstables::shared_sstable operator()() {
        auto sst = env.make_sstable(s, path, generation_from_value(gen));
...
sstables::shared_sstable create_sstable(sstables::test_env& env, simple_schema& sschema, const sstring& path) {
    ...
    return make_sstable_containing([&] {
            return env.make_sstable(sschema.schema(), path, generation_from_value(0));
        }
        , mutations);
}
    ...
    for (int run = 0; run < 100; ++run) {
        ...
        for (auto& mb: scenario.readers_data) {
            auto sst_factory = [table_schema, &env, &tmp, gen = ++gen] () {
                return env.make_sstable(std::move(table_schema), tmp.path().string(), generation_from_value(gen),
            ...
            if (dist(engine)) {
                included_gens.insert(generation_from_value(gen));
                if (mb.m) {
                    merged += *mb.m;
                }
            }
