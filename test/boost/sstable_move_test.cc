...
SEASTAR_THREAD_TEST_CASE(test_sstable_move) {
    ...
    for (auto i = 0; i < 2; i++) {
        ...
        auto new_dir = format("{}/gen-{}", fs::path(cur_dir).parent_path().native(), generation_from_value(gen));
    ...
    sst->close_files().get();
    sst = env.reusable_sst(uncompressed_schema(), new_dir, generation_from_value(gen)).get0();
}
...
SEASTAR_THREAD_TEST_CASE(test_sstable_move_replay) {
    ...
    do {
        ...
        touch_directory(new_dir).get();
        done = partial_create_links(sst, fs::path(new_dir), generation_from_value(gen), count++);
        sst->move_to_new_dir(new_dir, generation_from_value(gen), true).get();
        remove_file(cur_dir).get();
    } while (!done);
}
