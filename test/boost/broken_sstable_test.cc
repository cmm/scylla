...
static future<> broken_sst(sstring dir, generation_type generation, schema_ptr s, sstring msg, std::optional<sstring> sst_name,
...
static future<> broken_sst(sstring dir, generation_type generation, sstring msg, std::optional<sstring> sst_name = std::nullopt) {
...
SEASTAR_TEST_CASE(test_empty_index) {
  return sstables::test_env::do_with_async([&] (sstables::test_env& env) {
    auto s = schema_builder("test_ks", "test_table")
                 ...
                 .build();
    sstable_ptr sstp = env.reusable_sst(s, "test/resource/sstables/empty_index", generation_from_value(36), sstable_version_types::mc).get0();
...
SEASTAR_TEST_CASE(missing_column_in_schema) {
    ...
    return broken_sst("test/resource/sstables/incompatible_serialized_type", generation_from_value(122), s,
...
SEASTAR_TEST_CASE(invalid_boundary) {
    schema_ptr s = schema_builder("test_ks", "test_t")
                       ...
                       .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/invalid_boundary", generation_from_value(33), s,
...
SEASTAR_TEST_CASE(mismatched_timestamp) {
    schema_ptr s = schema_builder("test_ks", "test_table")
                       ...
                       .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/mismatched_timestamp", generation_from_value(122), s,
...
SEASTAR_TEST_CASE(broken_open_tombstone) {
    schema_ptr s = schema_builder("test_ks", "test_table")
                       ...
                       .with_column("val", utf8_type, column_kind::regular_column)
                       .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/broken_open_tombstone", generation_from_value(122), s,
...
SEASTAR_TEST_CASE(broken_close_tombstone) {
    schema_ptr s = schema_builder("test_ks", "test_table")
                       ...
                       .with_column("val", utf8_type, column_kind::regular_column)
                       .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/broken_close_tombstone", generation_from_value(122), s,
...
SEASTAR_TEST_CASE(broken_start_composite) {
    schema_ptr s =
        schema_builder("test_ks", "test_table")
            ...
            .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/broken_start_composite", generation_from_value(76), s,
...
SEASTAR_TEST_CASE(broken_end_composite) {
    schema_ptr s =
        schema_builder("test_ks", "test_table")
            ...
            .with_column("test_val", utf8_type, column_kind::clustering_key)
            .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/broken_end_composite", generation_from_value(76), s,
...
SEASTAR_TEST_CASE(static_mismatch) {
    schema_ptr s =
        schema_builder("test_foo_bar_zed_baz_ks", "test_foo_bar_zed_baz_table")
            ...
            .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/static_column", generation_from_value(58), s,
...
SEASTAR_TEST_CASE(static_with_clustering) {
    schema_ptr s =
        schema_builder("test_foo_bar_zed_baz_ks", "test_foo_bar_zed_baz_table")
            ...
            .build(schema_builder::compact_storage::no);
    return broken_sst("test/resource/sstables/static_with_clustering", generation_from_value(58), s,
...
SEASTAR_TEST_CASE(zero_sized_histogram) {
    return broken_sst("test/resource/sstables/zero_sized_histogram", generation_from_value(5),
...
SEASTAR_TEST_CASE(bad_column_name) {
    return broken_sst("test/resource/sstables/bad_column_name", generation_from_value(58),
