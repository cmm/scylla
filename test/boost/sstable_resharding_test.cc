...
void run_sstable_resharding_test() {
  ...
  for (const auto version : writable_sstable_versions) {
    ...
    // create sst shared by all shards
    {
        ...
        auto sst = env.make_sstable(s, tmp.path().string(), generation_type{0}, version, sstables::sstable::format_types::big);
        write_memtable_to_sstable_for_test(*mt, sst).get();
    }
    auto sst = env.reusable_sst(s, tmp.path().string(), generation_type{0}, version, sstables::sstable::format_types::big).get0();
      ...
      for (const auto version : writable_sstable_versions) {
        ...
        // created unshared sstable
        {
            ...
            auto sst_gen = [&env, single_sharded_s, &tmp, gen, version]() mutable {
                return env.make_sstable(single_sharded_s, tmp.path().string(), generation_type{(*gen)++}, version, big);
            };
