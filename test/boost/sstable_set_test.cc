...
SEASTAR_TEST_CASE(test_sstables_sstable_set_read_modify_write) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        auto sst1 = make_sstable_easy(env, tmp, std::move(mr), cfg, generation::from_value(gen++));
        ...
        auto sst2 = make_sstable_easy(env, tmp, std::move(mr), cfg, generation::from_value(gen++));
