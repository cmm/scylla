...
SEASTAR_THREAD_TEST_CASE(test_distributed_loader_with_incomplete_sstables) {
    ...
    auto temp_sst_dir = sst::temp_sst_dir(sst_dir, generation_type{2});
    touch_dir(temp_sst_dir);

    temp_sst_dir = sst::temp_sst_dir(sst_dir, generation_type{3});
    touch_dir(temp_sst_dir);
    auto temp_file_name = sst::filename(temp_sst_dir, ks, cf, sst::version_types::mc, generation_type{3}, sst::format_types::big, component_type::TemporaryTOC);
    touch_file(temp_file_name);

    temp_file_name = sst::filename(sst_dir, ks, cf, sst::version_types::mc, generation_type{4}, sst::format_types::big, component_type::TemporaryTOC);
    touch_file(temp_file_name);
    temp_file_name = sst::filename(sst_dir, ks, cf, sst::version_types::mc, generation_type{4}, sst::format_types::big, component_type::Data);
    touch_file(temp_file_name);

    do_with_cql_env_thread([&sst_dir, &ks, &cf, &require_exist] (cql_test_env& e) {
        require_exist(sst::temp_sst_dir(sst_dir, generation_type{2}), false);
        require_exist(sst::temp_sst_dir(sst_dir, generation_type{3}), false);

        require_exist(sst::filename(sst_dir, ks, cf, sst::version_types::mc, generation_type{4}, sst::format_types::big, component_type::TemporaryTOC), false);
        require_exist(sst::filename(sst_dir, ks, cf, sst::version_types::mc, generation_type{4}, sst::format_types::big, component_type::Data), false);
    }, db_cfg_ptr).get();
}
...
SEASTAR_THREAD_TEST_CASE(test_distributed_loader_with_pending_delete) {
    ...
    write_file(gen_filename(generation_type{2}, component_type::TOC), toc_text);
    touch_file(gen_filename(generation_type{2}, component_type::Data));
    write_file(pending_delete_dir + "/sstables-2-2.log",
               component_basename(generation_type{2}, component_type::TOC) + "\n");

    // Temporary log file with single entry
    write_file(pending_delete_dir + "/sstables-3-3.log.tmp",
               component_basename(generation_type{3}, component_type::TOC) + "\n");

    // Regular log file with multiple entries
    write_file(gen_filename(generation_type{4}, component_type::TOC), toc_text);
    touch_file(gen_filename(generation_type{4}, component_type::Data));
    write_file(gen_filename(generation_type{5}, component_type::TOC), toc_text);
    touch_file(gen_filename(generation_type{5}, component_type::Data));
    write_file(pending_delete_dir + "/sstables-4-5.log",
               component_basename(generation_type{4}, component_type::TOC) + "\n" +
               component_basename(generation_type{5}, component_type::TOC) + "\n");

    // Regular log file with multiple entries and some deleted sstables
    write_file(gen_filename(generation_type{6}, component_type::TemporaryTOC), toc_text);
    touch_file(gen_filename(generation_type{6}, component_type::Data));
    write_file(gen_filename(generation_type{7}, component_type::TemporaryTOC), toc_text);
    write_file(pending_delete_dir + "/sstables-6-8.log",
               component_basename(generation_type{6}, component_type::TOC) + "\n" +
               component_basename(generation_type{7}, component_type::TOC) + "\n" +
               component_basename(generation_type{8}, component_type::TOC) + "\n");
    ...
    do_with_cql_env_thread([&] (cql_test_env& e) {
        ...
        // Regular log file with single entry
        require_exist(gen_filename(generation_type{2}, component_type::TOC), false);
        require_exist(gen_filename(generation_type{2}, component_type::Data), false);
        ...
        // Regular log file with multiple entries
        require_exist(gen_filename(generation_type{4}, component_type::TOC), false);
        require_exist(gen_filename(generation_type{4}, component_type::Data), false);
        require_exist(gen_filename(generation_type{5}, component_type::TOC), false);
        require_exist(gen_filename(generation_type{5}, component_type::Data), false);
        require_exist(pending_delete_dir + "/sstables-4-5.log", false);

        // Regular log file with multiple entries and some deleted sstables
        require_exist(gen_filename(generation_type{6}, component_type::TemporaryTOC), false);
        require_exist(gen_filename(generation_type{6}, component_type::Data), false);
        require_exist(gen_filename(generation_type{7}, component_type::TemporaryTOC), false);
