...
public:
    ...
    future<> load_sstables(unsigned iterations) {
        _sst.push_back(_env.make_sstable(s, this->dir(), generation_type{0}, sstables::get_highest_sstable_version(), sstable::format_types::big));
    ...
    future<double> flush_memtable(int idx) {
        return seastar::async([this, idx] {
            ...
            auto sst = _env.make_sstable(s, dir(), generation_type{idx}, sstables::get_highest_sstable_version(), sstable::format_types::big, _cfg.buffer_size);
        ...
        return test_setup::create_empty_test_dir(dir()).then([this, idx] {
            return sstables::test_env::do_with_async_returning<double>([this, idx] (sstables::test_env& env) {
                auto sst_gen = [this, gen = make_lw_shared<unsigned>(idx)] () mutable {
                    return _env.make_sstable(s, dir(), generation_type{(*gen)++}, sstables::get_highest_sstable_version(), sstable::format_types::big, _cfg.buffer_size);
                };
