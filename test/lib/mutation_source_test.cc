...
static void test_streamed_mutation_forwarding_across_range_tombstones(tests::reader_concurrency_semaphore_wrapper& semaphore, populate_fn_ex populate) {
    ...
    // rt2 may or may not be emitted pre-fast-forward
    range_tombstone_list rt2_pre_ff1(*s);
    rd.may_produce_tombstones(position_range(rt2_range), {}, &rt2_pre_ff1);
    if (!rt2_pre_ff1.empty()) {
